...
def main():
    ...
    parser.add_argument("-a", metavar="alphabet_size", help="alphabet size (list (x,y,z,...) or range (x-z or x-z/step) ok)", default="2")
    parser.add_argument("-k", metavar="food_max_length", help="food molecule max length  (list or range ok)", default="2")
    parser.add_argument("-n", metavar='polymer_max_length', help="polymer max length  (list or range ok)", default="4")
    parser.add_argument("-m", metavar="mean_catalyzed", help="mean number of catalyzed reactions per molecule  (list or range ok)", default="2.0")
    parser.add_argument("-r", metavar="replicate", help="The replicate number/seed (list or range ok)", default="1")
    parser.add_argument("-o", metavar="output", help= "Output directory (or stdout)", default="stdout") #ZU MACHEN, stdout
    parser.add_argument("-f", metavar="file_name_template", help="file name template (use #a,#k,#n,#m,#r for parameters)", default="polymer_model_a#a_k#k_n#n_m#m_r#r.crs")
                ...
                for n in polymer_max_lengths:
                    for m in means:
                        for r in number_of_replicates:
                            ...
                            parameters = {"a":a, "k":k, "n":n, "m":m, "r":r}
                            
                            if arguments["o"] == "stdout":
                                filename = "stdout"
                            else:
                                file = replace_parameters(arguments["f"], parameters)
                                filename = os.path.join(arguments["o"], file)
                                os.makedirs(os.path.dirname(filename), exist_ok=True)
                            
                            if all([x != None for x in parameters.values()]):
                                res_reaction_system = apply(parameters, total_files_expected)
                            else:
                            ...
                            if ".zip" in filename:
                                ...
                                with open(filename, "w") as f:
                                    f.write(replace_parameters("# Polymer model a=#a k=#k n=#n m=#m r=#r:\n\n", parameters))
...
def apply(parameters:dict[str:int, str:int, str:int, str:float, str:int], tot_f:int) -> ReactionSystem:
    ...
    for polymer in polymers:
        ...
        while i < successes:
            ...
            if polymer in reaction.catalysts: continue
...
def parse_float_input(input:str) -> list|range:
    ...
    if "," in input:
        ...
        output = []
        if "/" in input:
            output_ints = [input.split("-")[0]].extend(input.split("-")[1].split("/"))
...
def parse_integer_input(input:str) -> list|range:
    ...
    if "," in input:
        ...
        output = []
        for o in output_ints:
            if o.strip().isdigit(): output.append(int(o.strip()))
    elif "-" in input:
        if "/" in input:
            output_ints = [input.split("-")[0]].extend(input.split("-")[1].split("/"))
...
def replace_parameters(input_str:str,parameters:dict[str:int, str:int, str:int, str:float, str:int])-> str:
...
def binom_dist(n:int ,p:float)-> list[float]:
    ...
    p_of_eq_k = []
    for k in range(n+1):
        p_of_eq_k.append(decimal.Decimal(math.comb(n,k)) * decimal.Decimal(p**k) * decimal.Decimal((1-p)**(n-k)))
        dist.append(sum(p_of_eq_k))
        if sum(p_of_eq_k) == sum(p_of_eq_k[:-1]): break #PROBLEMATISCH, might cause issues with runtime if float decides to become more precise
    return dist
